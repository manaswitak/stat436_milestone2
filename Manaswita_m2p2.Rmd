...
create_happiness_plot <- function(data, variable) {
  ...
  ggplot(data, aes_string(x = variable, y = "Happiness_Score")) +
    ...
    scale_color_gradient(low = "lavender", high = "purple", labels = label_number(accuracy = 1)) +
    labs(
      x = var_label,
      y = "Happiness Score",
    ...
    theme(
      ...
      legend.title = element_text(size = 14, margin = margin(b = 30, r = 10))
    )
}
...
ui <- fluidPage(
  ...
  # Padding/margins for visual clarity 
  tags$head(
    tags$style(HTML("
      body {
        padding-top: 20px;
  ...
  # UI for user variable selection
  sidebarLayout(
    sidebarPanel(
      selectInput(
        inputId = "variable",
