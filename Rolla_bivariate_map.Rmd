...
server = function(input, output, session) {
  ...
  output$variable_slider <- renderUI({
    var_col <- input$variable
    var_label <- names(which(c("Trust" = "Corruption", "Freedom" = "Freedom", "GDP" = "GDP") == var_col))
    
    sliderInput("var_range", paste0(var_label, ":"),
      min = min(world_data[[var_col]], na.rm = TRUE),
      max = max(world_data[[var_col]], na.rm = TRUE),
      value = range(world_data[[var_col]], na.rm = TRUE),
      width = "100%"
    )
  })
  ...
  filtered_data = reactive({
    ...
    world_data %>%
      filter(
        ...
        Rank >= input$rank[1],
        Rank <= input$rank[2],
        .data[[var_col]] >= input$var_range[1],
        .data[[var_col]] <= input$var_range[2]
      )
  })
  ...
  output$map = ggiraph::renderGirafe({
    ...
    if (nrow(d) > 0) {
      # Create variable value column (invert if Corruption for "trust")
      d$var_val <- if(var_col == "Corruption") -d[[var_col]] else d[[var_col]]
      
      d = bi_class(d, x = rank_best, y = var_val, style = "quantile", dim = 3)
      # Add tooltip text
      d$tooltip_text <- paste0(
        d$name, "\n",
        "Rank: ", d$Rank, "\n",
        "Happiness: ", round(d$Happiness.score, 2), "\n",
        var_label, ": ", round(d[[var_col]], 3)
      )
    }
    ...
    p <- ggplot() +
      ...
      labs(title = paste("World", var_label, "& Happiness -", input$year))
    ...
    legend <- bi_legend(
      ...
      dim = 3,
      xlab = "Better rank",
      ylab = paste("Higher", tolower(var_label)),
      size = 10
    )
